...
public class PageInterceptor implements Interceptor {
	...
	public StringBuilder buildPageSqlForMysql(String sql, Page page) {
		...
		String beginrow = String.valueOf((currentPage - 1) * page.getPageSize());
		pageSql.append(sql);
	...
	public StringBuilder buildPageSqlForOracle(String sql, Page page) {
		...
		int currentPage = (page.getCurrentPage() > 0) ? page.getCurrentPage() : 1;
		String beginrow = String.valueOf((currentPage - 1) * page.getPageSize());
		String endrow = String.valueOf(currentPage * page.getPageSize());
	...
	public void setProperties(Properties properties) {
		/*
		 * dialect = properties.getProperty("dialect"); if ( null == dialect || "".equals(dialect) ) { try { throw new
		 * PropertyException("dialect property is not found!"); } catch ( PropertyException e ) { e.printStackTrace(); }
		 * } // pageMethodPattern = p.getProperty("pageMethodPattern"); if ( dialect == null || dialect.equals("") ) {
		 * try { throw new PropertyException("pageMethodPattern property is not found!"); } catch ( PropertyException e
		 * ) { e.printStackTrace(); } }
		 */
	}
